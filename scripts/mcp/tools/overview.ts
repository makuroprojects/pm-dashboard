  ...
  register(server) {
    ...
    server.registerTool(
      ...
      async ({ projectId, includeArchived, limit }) => {
        ...
        if (projects.length === 0) {
          return jsonText({ count: 0, projects: [] })
        }
        ...
        return jsonText({ count: results.length, projects: results })
      },
    )
    ...
    server.registerTool(
      ...
      async ({ projectId, includeUnassigned, limit }) => {
        ...
        return jsonText({ count: rows.length, rows: rows.slice(0, limit) })
      },
    )
